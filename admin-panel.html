...
<div class="section" id="book">
  <div class="box">
    ...
    <form action="book_test_action.php" method="POST">
      <h3>Book Test</h3>

      <input placeholder="Candidate Name" name="candidate_name" required>

      <input type="email" placeholder="Candidate Email" name="candidate_email" required>
      ...
      <select name="test_time" required>
      ...
      <button class="black-btn" type="submit">BOOK TEST</button>
    </form>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ===============================
     BOOK TEST BUTTON WORKING
  =============================== */
  const bookBtn=document.getElementById("bookTestBtn");
  if(bookBtn){
  ...
  if(testDate){
    testDate.addEventListener("change",e=>{
      ...
      if(day===5||day===6||day===0){
        alert("❌ Friday, Saturday & Sunday not allowed");
        e.target.value="";
      }
    });
  }
  ...
  /* ===============================
     AUTO CANDIDATE + TRF NUMBER
  =============================== */
  const candNo=document.getElementById("candidateNumber");
  const trfNo=document.getElementById("trfNumber");

  function gen(prefix){
    return prefix+"-"+Math.floor(100000+Math.random()*900000);
  }
  if(candNo) candNo.value=gen("CAND");
  if(trfNo) trfNo.value=gen("TRF");

  /* ===============================
     PHOTO BOX – TAP OPEN CAMERA
  =============================== */
  ...
  /* ===============================
     UPDATE RESULT BUTTON WORKING
  =============================== */
  const updateBtn=document.getElementById("updateResultBtn");
  if(updateBtn){
    updateBtn.addEventListener("click",()=>{
      const inputs=document.querySelectorAll("#update input[type='text']");
      const pdf=document.querySelector("#update input[type='file']");
      let ok=true;
      inputs.forEach(i=>{ if(!i.value) ok=false; });
      if(!pdf||!pdf.files.length) ok=false;

      if(!ok){
        alert("❌ Please fill all fields & upload TRF PDF");
        return;
      }
...
/* ===============================
   GENERATE PDF (FORM ONLY)
=============================== */
function generatePDF(){
  const el=document.getElementById("trfForm");
  if(!el){ alert("TRF Form not found"); return; }
